<template>
  <div class="h-full flex-column">
    <el-form class="search-box" ref="searchFormRef" :model="searchForm" label-position="top">
      <el-row :gutter="30">
        <el-col :span="4">
          <el-form-item :label="$t('control.user.userConditionLabel')" prop="site">
            <el-input
              type="text"
              v-model="searchForm.site"
              :placeholder="$t('control.user.userConditionPlaceholder')"
              clearable
            />
          </el-form-item>
        </el-col>
    ...
    <div class="table-box flex-1 flex-column" v-loading="tableLoading">
      <div class="table flex-1 ovh">
        <el-table :data="tableData" border height="100%">
          ...
          <el-table-column :label="$t('control.user.userName')" prop="siteName" />
          <el-table-column :label="$t('control.user.account')" prop="host" />
          <el-table-column :label="$t('control.user.role')" prop="host" />
          <el-table-column :label="$t('control.user.recentLoginTime')" prop="host" />
          <el-table-column :label="$t('common.updateTime')" prop="updateTime" />
          <el-table-column :label="$t('common.updater')" prop="updater" />
          <el-table-column :label="$t('control.user.accessRestriction')" prop="host" />
          <el-table-column :label="$t('common.status')" prop="code" />
          <el-table-column :label="$t('common.operation')" width="250">
            <template #default>
              <el-button type="primary" link >编辑</el-button>
              <el-button type="primary" link @click="whiteListDialogRef?.show(1)">白名单</el-button>
    ...
    <!-- 白名单弹窗 -->
    <WhiteListDialog ref="whiteListDialogRef" />
  </div>
</template>
...
const whiteListDialogRef = useTemplateRef('whiteListDialogRef');
